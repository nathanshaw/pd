...
assignHH(2);
/////////////////////////////////////////////////////
for(1.0 => float timeScale; timeScale > 0.01; 0.91 *=> timeScale){
    setNoteLength(timeScale);
    if(maybe+maybe){intro(32, 16);};
    if(maybe+maybe){intro(32, Math.random2(0,5));};
    if(maybe+maybe){assignSnare(1);};
    if(maybe+maybe){verse(128, 12);};
    if(maybe+maybe){assignSnare(0);};
    if(maybe+maybe){bridge(128,16);};
    if(maybe+maybe){outro(128,12);};
    if(maybe+maybe){outro(32,Math.random2(0,9));};
    quarter => now;
